...
# Epoch Plotting Hook
##################################
class EpochPlottingHook(HookBase):
    """
    Hook que se ejecuta al final de cada epoch para printear un resumen
    y guardar un gráfico con la Train Loss y las métricas de Validación.
    """
    def __init__(self, iters_per_epoch: int, output_dir: str):
        self.iters_per_epoch = iters_per_epoch
    ...
    def after_step(self):
        # Comprobar si hemos llegado al final de un epoch exacto
        if (self.trainer.iter + 1) % self.iters_per_epoch == 0:
            self.epoch_count += 1
            
            # 1. Obtener Train Loss (suavizada con la mediana de las últimas 20 iteraciones)
            try:
                train_loss = self.trainer.storage.history("total_loss").median(20)
            except KeyError:
                train_loss = 0.0
            
            # 2. Obtener las últimas métricas de Validación
            val_miou = self._get_latest_metric("sem_seg/mIoU")
            val_pacc = self._get_latest_metric("sem_seg/pACC")
            val_macc = self._get_latest_metric("sem_seg/mACC")
            val_fwiou = self._get_latest_metric("sem_seg/fwIoU")
            
            # 3. Guardar en el historial
            self.history['train_loss'].append(train_loss)
            self.history['val_miou'].append(val_miou)
            self.history['val_pacc'].append(val_pacc)
            self.history['val_macc'].append(val_macc)
            self.history['val_fwiou'].append(val_fwiou)
            
            # 4. Printear resumen en consola
            print(f"\n" + "="*40)
            print(f"    RESUMEN EPOCH {self.epoch_count}:")
            print(f"    Train Loss: {train_loss:.4f}")
            print(f"    Val mIoU:   {val_miou:.2f}")
            print(f"    Val pAcc:   {val_pacc:.2f}")
            print(f"    Val mAcc:   {val_macc:.2f}")
            print(f"    Val fwIoU:  {val_fwiou:.2f}")
            print("="*40 + "\n")
            
            # 5. Dibujar y guardar la gráfica
            self.plot_history()
 
    def _get_latest_metric(self, name):
        """Intenta extraer la última métrica de EventStorage. Retorna 0.0 si no existe."""
        try:
            # history(name).values() devuelve una lista de tuplas: [(valor, iteración), ...]
            return self.trainer.storage.history(name).values()[-1][0]
        except (KeyError, IndexError):
            return 0.0
    ...
    def plot_history(self):
        ...
        # Train Loss
        plt.subplot(1, 3, 1)
        plt.plot(epochs_range, self.history['train_loss'], label='Train Loss', marker='o', markersize=4, color='tab:blue')
        plt.xlabel('Epoch'); plt.ylabel('Loss')
        plt.title('Train Loss Curve'); plt.legend(); plt.grid(True)
 
        # mIoU
        plt.subplot(1, 3, 2)
        plt.plot(epochs_range, self.history['val_miou'],  label='Val mIoU',  marker='s', markersize=4, color='tab:green')
        plt.xlabel('Epoch'); plt.ylabel('mIoU (0-100)')
        plt.title('Mean Intersection over Union'); plt.legend(); plt.grid(True)
 
        # Resto de Métricas (Accuracies)
        plt.subplot(1, 3, 3)
        plt.plot(epochs_range, self.history['val_pacc'],  label='Val pAcc',  marker='s', markersize=4)
        plt.plot(epochs_range, self.history['val_macc'],  label='Val mAcc',  marker='^', markersize=4)
        plt.plot(epochs_range, self.history['val_fwiou'], label='Val fwIoU', marker='D', markersize=4)
        plt.xlabel('Epoch'); plt.ylabel('Score (0-100)')
        plt.title('Accuracies & fwIoU'); plt.legend(); plt.grid(True)
 
        plt.tight_layout()
        save_path = os.path.join(self.output_dir, "training_history.png")
        print(f"Saving training graph: {save_path}")
        plt.savefig(save_path)
        plt.close()

##################################
# Fin Epoch Plotting Hook
##################################
...
def main(args):
    ...
    # VALIDATION
    dataset_path_image = VAL_IMAGES_PATH
    dataset_path_png_mask = VAL_PNG_MASKS_PATH
    dataset_path_tif_mask = '' # Not used yet, for panoptic segmentation

    DatasetCatalog.register("rios_val", lambda : get_gaofen_dict(dataset_path_image,dataset_path_png_mask,dataset_path_tif_mask))
    MetadataCatalog.get("rios_val").stuff_classes = ALL_CLASSES
    MetadataCatalog.get("rios_val").ignore_label = 0
    MetadataCatalog.get("rios_val").thing_dataset_id_to_contiguous_id = stuff_dataset_id_to_contiguous_id
