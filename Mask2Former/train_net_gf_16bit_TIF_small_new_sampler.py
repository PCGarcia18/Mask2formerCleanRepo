...
class Trainer(DefaultTrainer):
    ...
    def build_train_loader(cls, cfg):
        # 1. Setup the Mapper (Your existing custom mapper)
        # ------------------------------------------------
        ...
        # 2. Get the dataset dictionaries
        # ------------------------------------------------
        ...
        if cfg.DATALOADER.SAMPLER_TRAIN == "RepeatFactorTrainingSampler":
            ...
            for d in tqdm(dataset_dicts):
                ...
                if "annotations" not in d:
                    ...
                    # Read mask (ensure flags match your 16bit/8bit data needs)
                    # changing to IMREAD_UNCHANGED to support 16-bit if needed
                    mask = cv2.imread(mask_file, cv2.IMREAD_UNCHANGED)
                    
                    if mask is not None:
                        # Find all unique class IDs in this image
            ...
            # 4. Calculate Repeat Factors
            # ------------------------------------------------
            # This uses the static method from the file you pasted (Line 118)
        ...
        # 5. Build the Loader with the specific sampler
        # ------------------------------------------------
        return build_detection_train_loader(
            cfg, 
            mapper=mapper, 
            sampler=sampler,  # Pass our custom sampler here
            dataset=dataset_dicts # Pass the dicts (now containing 'annotations')
        )
    @classmethod
