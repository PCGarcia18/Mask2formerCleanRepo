...
def get_mask2former_flops(model):
    from fvcore.nn import FlopCountAnalysis, flop_count_table
    # Ensure model is in eval mode
    model.eval()
    
    dummy_input = [{
        "image": torch.randn(4, 800, 845).cuda(),  
...
def inference_on_dataset(
    ...
    #flops = get_mask2former_flops(model)
    #print(f"Total GFLOPs: {flops/1e9:.2f}G")
    #### END EXTRA PABLO FLOPS
