...
"""
python demo_mod_full_size_hyper.py --config-file /home/pablo.canosa/wip/Mask2Former/output/config.yaml --input /home/pablo.canosa/wip/datasets/split_gaofen/test_16bit/GF2_PMS1__L1A0001064454-MSS1.tiff --opts MODEL.WEIGHTS /home/pablo.canosa/wip/Mask2Former/output/model_0004999.pth 
"""
...
from mask2former import (
    ...
    MaskFormerSemanticDatasetMapperGDAL,
...
class Trainer(DefaultTrainer):
    ...
    def build_evaluator(cls, cfg, dataset_name, output_folder=None):
        ...
        if evaluator_type == "sem_seg_GDAL":# Mi version modificada
            evaluator_list.append(
                SemSegEvaluatorGDAL(
                    dataset_name,
                    distributed=True,
    ...
    def build_train_loader(cls, cfg):
        ...
        # Semantic segmentation dataset mapper for GDAL
        elif cfg.INPUT.DATASET_MAPPER_NAME == "mask_former_semantic_GDAL":
            mapper = MaskFormerSemanticDatasetMapperGDAL(cfg, True)
            return build_detection_train_loader(cfg, mapper=mapper)
...
ID_TO_COLOR_DICT = {# Este diccionario solo se usa para la segmentación panoptica
    0: (0, 0, 0),
    1: (200, 0, 0),
    2: (0, 200, 0),
    3: (150, 250, 0),
...
def get_gaofen_dict(images_path, gt_dir_png, gt_dir_tif_color):
    ...
    for image_idx, image_filename in enumerate(os.listdir(images_path)):
        ...
        image_id, _= os.path.splitext(image_filename)# eliminar la extension para tener el image_id 
        ...
        #######
        #######
        # ESTE CÓDIGO ES PARA HACER LA CARGA DE LAS MÁSCARAS DE SEGMENTACIÓN PANÓPTICA EN COLOR
        #######
        #######
...
def main(args):
    ...
    ###
    ###
    thing_dataset_id_to_contiguous_id = {i: i for i in range(25)}# Este es un apaño temporal, como todas las clases se consideran como things el diccionario es trivial, 0:0,1:1 ,..., 24:24
    
    # This are the paths for the dataset files in quadrants
    dataset_path_image = '/home/pablo.canosa/wip/datasets/split_gaofen/train_16bit/' 
    dataset_path_png_mask = '/home/pablo.canosa/wip/datasets/split_gaofen/train_grayscale_masks/'
    dataset_path_tif_mask = '/home/pablo.canosa/wip/datasets/split_gaofen/train_color_masks/'
    ...
    MetadataCatalog.get("gaofen_train").thing_classes  = ALL_CLASSES
    #MetadataCatalog.get("gaofen_train").stuff_classes = ALL_CLASSES
    MetadataCatalog.get("gaofen_train").ignore_label = 0 # esto es para que ignore el fondo, los pixeles que no tienen clase asignada y tienen el vlaor 0 en la mascara en escala de grises

    MetadataCatalog.get("gaofen_train").thing_dataset_id_to_contiguous_id = thing_dataset_id_to_contiguous_id
    
    
    MetadataCatalog.get("gaofen_train").thing_colors = COLOR_LIST
    MetadataCatalog.get("gaofen_train").stuff_colors = COLOR_LIST
    #MetadataCatalog.get("gaofen_train").stuff_classes = ALL_CLASSES # según Álvaro deberia considerar todas como stuff
    



    # test trials
    dataset_path_image_test = '/home/pablo.canosa/wip/datasets/split_gaofen/test_16bit/' 
    dataset_path_png_mask_test = '/home/pablo.canosa/wip/datasets/split_gaofen/test_grayscale_masks/'
    dataset_path_tif_mask_test = '/home/pablo.canosa/wip/datasets/split_gaofen/test_color_masks/'
    ...
    MetadataCatalog.get("gaofen_test").thing_classes  = ALL_CLASSES
    MetadataCatalog.get("gaofen_test").ignore_label = 0 # esto es para que ignore el fondo, los pixeles que no tienen clase asignada y tienen el vlaor 0 en la mascara en escala de grises
    MetadataCatalog.get("gaofen_test").stuff_classes = ALL_CLASSES # Me la pide el evaluator, no entiendo porque si esto coon segmentación semántica y teóricamente no haría falta
    MetadataCatalog.get("gaofen_test").thing_dataset_id_to_contiguous_id = thing_dataset_id_to_contiguous_id
    MetadataCatalog.get("gaofen_test").evaluator_type = "sem_seg_GDAL"# este con esta version falla si no se añade stuff a la metdata
    MetadataCatalog.get("gaofen_test").thing_colors = COLOR_LIST
